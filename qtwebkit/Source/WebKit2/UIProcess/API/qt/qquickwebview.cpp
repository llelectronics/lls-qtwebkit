...
void QQuickWebViewLegacyPrivate::updateViewportSize()
{
    ...
    if (DrawingAreaProxy *drawingArea = webPageProxy->drawingArea()) {
        ...
        drawingArea->setSize((viewportSize / devicePixelRatio).toSize(), IntSize(), IntSize());
        drawingArea->setVisibleContentsRect(FloatRect(FloatPoint(), FloatSize(viewportSize / devicePixelRatio)), FloatPoint());
    }
}
